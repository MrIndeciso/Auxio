...
class SearchSettingsImpl @Inject constructor(@ApplicationContext context: Context) :
    ...
    override fun migrate() {
        val newKey = getString(R.string.set_key_search_filters)
        // Keep the old key for recovery, and never overwrite a user's new filter selection.
        if (
            sharedPreferences.contains(newKey) || !sharedPreferences.contains(OLD_KEY_SEARCH_FILTER)
        )
            return
